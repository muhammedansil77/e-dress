...
export class JwtUtils {
  ...
  public static generateCustomerToken(payload: JwtCustomerPayload): string {
    const options: SignOptions = {
      expiresIn: (env.JWT_ACCESS_EXPIRES_IN || '7d') as any,
    };
    return jwt.sign(payload, env.JWT_ACCESS_SECRET, options);
  }

  public static verifyCustomerToken(token: string): JwtCustomerPayload {
    return jwt.verify(token, env.JWT_ACCESS_SECRET) as JwtCustomerPayload;
  }
}
